...
const BookingsPage = () => {
    ...
    try {
      ...
      const bookingsWithRooms = await Promise.all(
        allBookings.map(async (booking: Booking) => {
          try {
            const detailResponse = await api.get(`/bookings/${booking.id}`);
  ...
  const filterBookings = () => {
    ...
    switch (filter) {
      case 'upcoming':
        return bookings.filter(b => new Date(b.start_time) > now && b.status !== 'cancelled');
      case 'past':
        return bookings.filter(b => new Date(b.end_time) < now || b.status === 'cancelled');
      default:
        return bookings;
    }
  };
